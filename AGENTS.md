...
- Crawlee Cloud platform service: resource `encuentra24-crawlee-cloud`, UUID `bdvayjem0bz9pes8fmt5rc19`, domains `crawlee.llermaly.com` and `crawlee-dashboard.llermaly.com`. Status as of 2026-05-09: stopped after initial start made the Coolify panel/API intermittently unavailable. The stopped service definition patches the runner container at runtime to skip broad Docker cleanup calls, but do not restart without a controlled maintenance window and live monitoring.
